...
class CNNDailyDataset(Dataset):
    ...
    def __init__(self, path, transforms, vocabs):
        ...
        for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            if len(article) > 512:
              article = article[:255] + article[-255:]
            self.srcs.append(article)

            highlight = self.transform_trg(row['highlights'])
            if len(highlight) > 512:
              highlight = highlight[:255] + highlight[-255:]
            self.trgs.append(highlight)
...
print(train_dataset.vocab_stoi)
exit()
...
SRC_PAD_IDX = train_dataset.vocab_stoi["<pad>"]
TRG_PAD_IDX = train_dataset.vocab_stoi["<pad>"]
...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for batch in iterator.batches:
        ...
        output, _ = model(src_batch, trg_batch[:,:-1])
